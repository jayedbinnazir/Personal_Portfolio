...
export const login = (email, password) => async (dispatch) => {
  ...
  try {
    const { data } = await axios.post(
      `${import.meta.env.VITE_APP_URL}/api/v1/user/login`,
      { email, password },
...
export const logout = () => async (dispatch) => {
  try {
    const { data } = await axios.get(
      `${import.meta.env.VITE_APP_URL}/api/v1/user/logout`,
      { withCredentials: true }
    );
  ...
  (currentPassword, newPassword, confirmNewPassword) => async (dispatch) => {
    ...
    try {
      const { data } = await axios.put(
        `${import.meta.env.VITE_APP_URL}/api/v1/user/password/update`,
...
export const updateProfile = (data) => async (dispatch) => {
  ...
  try {
    const response = await axios.put(
      `${import.meta.env.VITE_APP_URL}/api/v1/user/me/profile/update`,
      data,
      {
        withCredentials: true,
