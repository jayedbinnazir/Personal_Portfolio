...
export const getAllTimeline = () => async (dispatch) => {
  ...
  try {
    const response = await axios.get(
      `${import.meta.env.VITE_APP_URL}/api/v1/timeline/getall`,
...
export const addNewTimeline = (data) => async (dispatch) => {
  ...
  try {
    const response = await axios.post(
      `${import.meta.env.VITE_APP_URL}/api/v1/timeline/add`,
      data,
      {
        withCredentials: true,
...
export const deleteTimeline = (id) => async (dispatch) => {
  ...
  try {
    const response = await axios.delete(
      `${import.meta.env.VITE_APP_URL}/api/v1/timeline/delete/${id}`,
      {
        withCredentials: true,
      }
    );
