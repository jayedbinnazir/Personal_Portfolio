...
export const getAllProjects = () => async (dispatch) => {
  ...
  try {
    const response = await axios.get(
      `${import.meta.env.VITE_APP_URL}/api/v1/project/getall`,
...
export const addNewProject = (data) => async (dispatch) => {
  ...
  try {
    const response = await axios.post(
      `${import.meta.env.VITE_APP_URL}/api/v1/project/add`,
      data,
      {
        withCredentials: true,
...
export const deleteProject = (id) => async (dispatch) => {
  ...
  try {
    const response = await axios.delete(
      `${import.meta.env.VITE_APP_URL}/api/v1/project/delete/${id}`,
      {
        withCredentials: true,
      }
    );
...
export const updateProject = (id, newData) => async (dispatch) => {
  ...
  try {
    const response = await axios.put(
      `${import.meta.env.VITE_APP_URL}/api/v1/project/update/${id}`,
      newData,
      {
        withCredentials: true,
        headers: { "Content-Type": "multipart/form-data" },
