...
export const register = catchAsyncErrors(async (req, res, next) => {
  ...
  const user = await User.create({
    ...
    linkedInURL,
    avatar: {
      public_id: cloudinaryResponseForAvatar.public_id, // Corrected here
      url: cloudinaryResponseForAvatar.secure_url, // Corrected here
    },
    resume: {
      public_id: cloudinaryResponseForResume.public_id, // Corrected here
      url: cloudinaryResponseForResume.secure_url, // Corrected here
    },
  });
...
export const updatePassword = catchAsyncErrors(async (req, res, next) => {
  ...
  user.password = newPassword; // Removed await
  await user.save();
  res.status(200).json({
    success: true,
...
export const getUserForPortfolio = catchAsyncErrors(async (req, res, next) => {
  const id = "66fe3664af3972c5ef401417";
...
export const resetPassword = catchAsyncErrors(async (req, res, next) => {
  ...
  user.password = req.body.password; // Removed await
  user.resetPasswordToken = undefined;
  user.resetPasswordExpire = undefined;
  await user.save();
  res.status(200).json({
